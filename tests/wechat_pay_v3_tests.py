...
from wechat_pay_v3 import WechatPayV3
...
class WechatPayV3EncryptTest(WechatPayV3Test):
    ...
    def test_aes_decrypt(self):
        ...
        cipherbyte = wp._encrypt_manager._aes_encrypt(
            wp._api_v3_key, data, nonce_str, associated_data)
...
class WechatPayV3CoreTest(WechatPayV3Test):
    def test_build_header_auth_token(self):
        ...
        token = wp._core._build_header_auth_token(method, relative_url, timestamp, nonce_str,
                                                  request_body)
        self.assertEqual(
            token,
